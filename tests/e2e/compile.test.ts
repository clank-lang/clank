 ...
 *
 * Tests the full compilation pipeline from Clank source to JavaScript.
 */
...
/**
 * Compile and run Clank code by writing to a temp file and importing it.
...
async function compileAndRun(source: string): Promise<unknown> {
  ...
  const tempPath = `/tmp/clank-test-${Date.now()}-${Math.random().toString(36).slice(2)}.mjs`;
...
describe("E2E: Code Quality", () => {
  ...
  test("generates runtime for Option/Result types", () => {
    ...
    `);
    expect(result.success).toBe(true);
    expect(result.code).toContain("__clank");
  });
